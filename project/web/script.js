...
function enabled_function() {
  ...
  if(document.getElementById("lisence").checked === true){
    ...
    document.getElementById("submit").style.boxShadow = '1px 1px 5px 1px rgb(173, 174, 236),1px -1px 7px 2px rgb(152, 153, 231)';
  } else {
...
function send_comment_function() {
  count = count - 1;
   ...
   if(count <= 8){
    swal("Sorry !!!!", "One User Send Only 1 Comment", "warning"); 
  } else {
    swal("Success", "", "success");
  }
...
var totleLength = document.body.scrollHeight - window.innerHeight;
 
  var progressbarHeight = (window.pageYOffset / totleLength) * 100;
  progress.style.height = progressbarHeight + '%';
